...
export class SkillScene extends Phaser.Scene {
    ...
    _buildUI() {
        ...
        // Owned section
        let y = 50;
        this.add.text(20, y, 'APRENDIDAS', { fontSize: '11px', color: '#88ff88', fontFamily: 'Courier New', fontStyle: 'bold' }).setOrigin(0, 0);
        y += 18;
        const owned = this._player.skills || [];
        if (owned.length === 0) {
            this.add.text(20, y, '(nenhuma ainda)', { fontSize: '10px', color: '#666666', fontFamily: 'Courier New' });
            y += 16;
        } else {
            owned.forEach(id => {
                const s = SKILLS[id]; if (!s) return;
                this.add.rectangle(20, y, 504, 18, 0x0e1a0e, 1).setOrigin(0, 0).setStrokeStyle(1, 0x336633);
                this.add.text(26, y + 9, `[T${s.tier}] ${s.name}`, { fontSize: '10px', color: '#88ff88', fontFamily: 'Courier New', fontStyle: 'bold' }).setOrigin(0, 0.5);
                this.add.text(150, y + 9, s.description, { fontSize: '9px', color: '#aaaaaa', fontFamily: 'Courier New' }).setOrigin(0, 0.5);
                y += 22;
            });
        }
        ...
        if (pending.length === 0) {
            this.add.text(W / 2, y, 'Nenhuma habilidade nova disponível.\nSuba de nível para desbloquear mais.', {
                fontSize: '11px', color: '#666666', fontFamily: 'Courier New', align: 'center',
            }).setOrigin(0.5, 0);
            return;
        }

        this.add.text(20, y, 'ESCOLHA UMA HABILIDADE', { fontSize: '11px', color: '#ffd700', fontFamily: 'Courier New', fontStyle: 'bold' });
        y += 22;

        for (const tier of pending) {
            this.add.text(20, y, `Tier ${tier.tier}  (escolha 1):`, { fontSize: '10px', color: '#aaaaff', fontFamily: 'Courier New' });
            y += 16;
            for (const s of tier.options) {
                ...
                this.add.text(28, y + 8, `${s.name}`, { fontSize: '11px', color: '#88ccff', fontFamily: 'Courier New', fontStyle: 'bold' });
                this.add.text(28, y + 23, s.description, { fontSize: '9px', color: '#aaaaaa', fontFamily: 'Courier New', wordWrap: { width: 470 } });
                y += 44;
                if (y > 430) break;
            }
            if (y > 430) break;
